...
const dataPath = process.env.DATA_FAD_PATH || 'c:/MyLocal/Data/DataFad/dataFad.json'
...
async function main() {
  ...
  const vendorPath = process.env.DATA_VENDOR_PATH || 'c:/MyLocal/Data/DataFad/dataVendor.json'
  let vendorFile = []
  try {
    const vraw = await fs.readFile(vendorPath, 'utf8')
    vendorFile = JSON.parse(vraw)
    console.log(`Loaded ${vendorFile.length} vendor records from ${vendorPath}`)
  } catch (e) {
  ...
  for (const r of arr) {
    try {
      ...
      if (vendorName) {
        if (vendorCache.has(vendorName)) {
          ...
          const vendorMeta = vendorFile.find(
            (x) => (x.name || x.vendor) && (x.name || x.vendor).trim() === vendorName,
          )
          const createData = { name: vendorName, active: vendorMeta?.active ?? true }
      ...
      inserted++
      if (inserted % 100 === 0) console.log(`Inserted/updated ${inserted} records`)
    } catch (e) {
