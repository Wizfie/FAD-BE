...
export class ProgramInfoService {
  ...
  static async convertHeicToJpeg(file) {
    const srcPath = path.join(env.UPLOAD_DIR, file.filename);
  ...
  static async uploadImage(file, title = null, displayOrder = 0) {
    try {
      ...
      const oldPath = path.join(env.UPLOAD_DIR, file.filename);
      const newPath = path.join(env.UPLOAD_DIR, newFilename);
      ...
      const thumbPath = path.join(env.UPLOAD_DIR, thumbFilename);

      await makeThumb(newPath, thumbPath, file.mimetype);

      // Generate URLs
      const baseUrl = `${env.BASE_URL}/uploads`;
      const url = `${baseUrl}/${newFilename}`;
      const thumbUrl = `${baseUrl}/${thumbFilename}`;
  ...
  static async deleteImage(imageId) {
    try {
      ...
      const imagePath = path.join(env.UPLOAD_DIR, image.filename);
      const thumbPath = path.join(env.UPLOAD_DIR, image.thumbFilename);
