...
export class ProgramInfoController {
  ...
  static async upload(req, res) {
    try {
      ...
      await changeLog(
        "ProgramInfoImage",
        "CREATE",
        { id: result.id, filename: result.filename, title: result.title },
        req.user
      );
  ...
  static async remove(req, res) {
    try {
      ...
      await changeLog(
        "ProgramInfoImage",
        "DELETE",
        { id: existingImage.id, filename: existingImage.filename },
        req.user
      );
  ...
  static async updateOrder(req, res) {
    try {
      ...
      await changeLog(
        "ProgramInfoImage",
        "UPDATE_ORDER",
        { count: imageOrders.length, imageOrders },
        req.user
      );
  ...
  static async reorder(req, res) {
    try {
      logger.request(req, "↔️ Reorder single program info image");

      const { id } = req.params;
      const { newDisplayOrder } = req.body;
      const imageId = parseInt(id);

      if (!imageId || newDisplayOrder === undefined) {
        return res.status(400).json({
          success: false,
          message: "ID gambar atau display order tidak valid",
        });
      }

      const updatedImage = await prisma.programInfoImage.update({
        where: { id: imageId },
        data: { displayOrder: parseInt(newDisplayOrder) },
      });
      ...
      await changeLog(
        ...
        {
          id: imageId,
          newDisplayOrder: parseInt(newDisplayOrder),
        },
        req.user
      );

      logger.info("Program info image reordered", {
        id: imageId,
        newDisplayOrder: parseInt(newDisplayOrder),
      });
      ...
      logger.error("Failed to reorder program info image", error);
      res.status(500).json({
        success: false,
        message: error.message || "Gagal mengubah urutan gambar",
      });
    }
  }
}
